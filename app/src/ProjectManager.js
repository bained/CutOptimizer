'use strict';

import { Part } from './model/Part.js';
import { Optimizer } from './engine/Optimizer.js';
import { ConfigManager } from './ConfigManager.js';

/**
 * ProjectManager — управлява жизнения цикъл на един проект.
 * Отговаря за зареждане на JSON, стартиране на оптимизация и запазване на резултати.
 */
class ProjectManager {
    /**
     * @param {string} configPath - Път до config.ini.
     */
    constructor(configPath) {
        this.config = new ConfigManager(configPath);
        this.optimizer = null;
        this.parts = [];
        this.sheetW = 0;
        this.sheetH = 0;
        this.kerf = 0;
        this.result = null;
        this.projectFilePath = '';
    }

    /**
     * Парсва JSON стойност, която може да е string или number.
     * @param {*} val - Стойност от JSON.
     * @returns {number}
     */
    parseNumericValue(val) {
        if (typeof val === 'string') {
            return parseInt(val, 10);
        }
        return val;
    }

    /**
     * Зарежда проект от JSON файл (Neutralino API).
     * @param {string} filePath - Път до .json файла.
     * @returns {Promise<Object>} - Метаданни за заредения проект.
     */
    async loadProject(filePath) {
        try {
            const content = await Neutralino.filesystem.readFile(filePath);
            const data = JSON.parse(content);

            this.sheetW = this.parseNumericValue(data.sheetW);
            this.sheetH = this.parseNumericValue(data.sheetH);
            this.kerf = this.parseNumericValue(data.kerf);

            this.parts = [];
            for (let i = 0; i < data.parts.length; i++) {
                const p = data.parts[i];
                const part = new Part(
                    p.n,
                    this.parseNumericValue(p.w),
                    this.parseNumericValue(p.h),
                    this.parseNumericValue(p.r) === 1
                );
                this.parts.push(part);
            }

            this.projectFilePath = filePath;
            this.result = null;

            // Създаваме Optimizer-а с параметрите от JSON
            this.optimizer = new Optimizer(this.sheetW, this.sheetH, this.kerf);

            // Зареждаме и config за параметри като iterations, beamWidth
            await this.config.load();

            return {
                sheetW: this.sheetW,
                sheetH: this.sheetH,
                kerf: this.kerf,
                partCount: this.parts.length
            };
        } catch (err) {
            console.error('ProjectManager: Failed to load project.', err.message);
            throw err;
        }
    }

    /**
     * Стартира оптимизацията с текущите части и настройки.
     * @returns {Promise<Object>} - Резултат от оптимизацията.
     */
    async runOptimization() {
        if (!this.optimizer) {
            throw new Error('ProjectManager: No project loaded. Call loadProject() first.');
        }

        if (this.parts.length === 0) {
            throw new Error('ProjectManager: No parts to optimize.');
        }

        const settings = this.config.get();
        const iterations = settings.iterations;
        const beamWidth = settings.beamWidth;

        // Стартираме оптимизацията (използваме setTimeout, за да не блокираме UI)
        const result = await new Promise((resolve, reject) => {
            try {
                const layout = this.optimizer.optimize(this.parts, iterations, beamWidth);
                resolve(layout);
            } catch (err) {
                reject(err);
            }
        });

        this.result = result;

        return this.getResultSummary();
    }

    /**
     * Връща резюме на резултата.
     * @returns {Object}
     */
    getResultSummary() {
        if (!this.result) {
            return null;
        }

        const totalParts = this.countTotalPlacedParts();
        let totalEff = 0;
        for (let i = 0; i < this.result.sheets.length; i++) {
            totalEff += this.result.sheets[i].efficiency;
        }
        const avgEfficiency = this.result.sheets.length > 0
            ? (totalEff / this.result.sheets.length)
            : 0;

        return {
            sheetCount: this.result.sheets.length,
            totalPlacedParts: totalParts,
            avgEfficiency: avgEfficiency,
            score: this.result.score
        };
    }

    /**
     * Пресмята общия брой поставени части във всички листове.
     * @returns {number}
     */
    countTotalPlacedParts() {
        let count = 0;
        for (let i = 0; i < this.result.sheets.length; i++) {
            count += this.result.sheets[i].parts.length;
        }
        return count;
    }

    /**
     * Запазва резултата като JSON (Neutralino API).
     * @param {string} outputPath - Път за запис.
     * @returns {Promise<void>}
     */
    async saveResult(outputPath) {
        if (!this.result) {
            throw new Error('ProjectManager: No result to save.');
        }

        const output = {
            input: {
                sheetW: this.sheetW,
                sheetH: this.sheetH,
                kerf: this.kerf,
                partCount: this.parts.length
            },
            result: this.getResultSummary(),
            sheets: []
        };

        for (let si = 0; si < this.result.sheets.length; si++) {
            const s = this.result.sheets[si];
            const sheetData = {
                efficiency: s.efficiency,
                parts: []
            };

            for (let pi = 0; pi < s.parts.length; pi++) {
                const p = s.parts[pi];
                sheetData.parts.push({
                    name: p.name,
                    x: p.x,
                    y: p.y,
                    w: p.w,
                    h: p.h,
                    rotated: p.rotated ? 1 : 0
                });
            }

            output.sheets.push(sheetData);
        }

        try {
            await Neutralino.filesystem.writeFile(outputPath, JSON.stringify(output, null, 2));
        } catch (err) {
            console.error('ProjectManager: Failed to save result.', err.message);
            throw err;
        }
    }

    /**
     * Връща пълния Layout резултат.
     * @returns {import('./engine/Optimizer.js').Layout|null}
     */
    getLayout() {
        return this.result;
    }
}

export { ProjectManager };